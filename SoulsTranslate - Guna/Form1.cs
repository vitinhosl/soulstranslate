...
namespace SoulsTranslate
{
    ...
    {
        private string backupFirstText = null;
        private string originalFirstText = null;
        ...
        private string BuildRtf(string path, HashSet<string> highlightIds)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (mXml.Success)
                {
                    ...
                    string attributes = mXml.Groups[1].Value; // Captura tudo: version="1.0" encoding="utf-8"
                    var attrMatches = rxAttr.Matches(attributes);

                    for (int j = 0; j < attrMatches.Count; j++)
                    {
                        var a = attrMatches[j];
                        sb.Append(@" \cf3 ").Append(EscapeRtf(a.Groups[1].Value)) // chave (ex: version)
                          .Append(@"\cf1=").Append(@"\cf4 ").Append(EscapeRtf(a.Groups[2].Value)); // valor (ex: "1.0")

                        // Adiciona espaço entre atributos, exceto no último
                        if (j < attrMatches.Count - 1)
                            sb.Append(@"\cf1 ");
                    }
        ...
        private async Task LoadFirstFile(string path)
        {
            ...
            // 1) Store the raw XML content for search restoration
            originalFirstText = File.ReadAllText(path, Encoding.UTF8);

            // 2) Execute parsing in background
            ...
            // 3) Build RTF in background
            string rtf = await Task.Run(() => BuildRtf(path));

            // 4) Apply to UI all at once
        ...
        {
            if (backupFirstText == null)
            {
                backupFirstText = originalFirstText ?? txtFirst.Text;
            }
        ...
        {
            // -------------------------------------------------------
            // 3) Se já existir backup, restaure-o e reaplique RTF + gutter
            // -------------------------------------------------------
            if (backupFirstText != null)
            {
                // 3.1) Restaura o conteúdo original no txtFirst
                txtFirst.Text = backupFirstText;

                try
                {
                    // 3.2) Para reaplicar a coloração RTF, gravamos temporariamente o XML e chamamos BuildRtf
                    var temp = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".xml");
                    File.WriteAllText(temp, txtFirst.Text, Encoding.UTF8);

                    // 3.3) Gera o RTF sem nenhum highlight (usamos a sobrecarga que não recebe highlightIds)
                    string rtfOriginal = BuildRtf(temp);

                    // 3.4) Coloca o RTF de volta no txtFirst
                    FastPopulateByRtf(txtFirst, rtfOriginal);

                    // 3.5) Atualiza o gutter para sincronizar os números de linha
                    UpdateGutter(txtFirst, txtFirstNumber);

                    // 3.6) Deleta o arquivo temporário
                    File.Delete(temp);
                }
                catch
                {
                    // Se der erro ao gerar RTF, ao menos deixamos o texto “puro” restaurado.
                }

                // 3.7) Limpa o backup, para não permitir remover duas vezes seguidas
                backupFirstText = null;
            }
            else
            {
                MessageBox.Show("Não há nenhuma modificação de 'Add' para desfazer.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        private void btnSave_Click(object sender, EventArgs e)
        {
            // 1) Se não existir um arquivo carregado, avise ou abra SaveFileDialog
            if (string.IsNullOrEmpty(lastLoadedFilePath))
            {
                MessageBox.Show("Nenhum arquivo foi carregado em txtFirst. " + "Use 'Abrir...' ou arraste um XML primeiro.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning
                );
                return;
            }
            ...
            try
            {
                // 2) Se o arquivo original existe em disco, fazemos o backup
                if (File.Exists(lastLoadedFilePath))
                {
                    // monta o nome de backup: ex: "C:\pasta\meuarquivo.xml.prev"
                    string backupPath = lastLoadedFilePath + ".prev";

                    // Se já existir um backup anterior, apagamos para substituirmos
                    if (File.Exists(backupPath))
                    {
                        File.Delete(backupPath);
                    }

                    // Move o arquivo original para .prev
                    File.Move(lastLoadedFilePath, backupPath);
                }

                // 3) Agora gravamos o conteúdo atual do txtFirst de volta no caminho original
                //    Garantimos salvar com codificação UTF-8 (ou a codificação desejada)
                File.WriteAllText(lastLoadedFilePath, txtFirst.Text, Encoding.UTF8);

                MessageBox.Show("Arquivo salvo com sucesso! " + "Backup: " + Path.GetFileName(lastLoadedFilePath) + ".prev", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
